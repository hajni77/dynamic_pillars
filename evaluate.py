...
def do_eval(det_results, gt_results, CLASSES, saved_path):
    ...
    overall_results = {}
    wrong_class_detections = {cls: 0 for cls in CLASSES}
                        ...
                        if match_id != -1:
                            ...
                            if det_ignores[match_id] == 0 and gt_ignores[j] == 0:
                                tp_scores.append(match_score)
                                                            # Check if the class is wrong
                                # Check if the class is wrong
                                if match_id < len(cur_det_names) and cur_det_names[match_id] != cur_gt_names[j]:
                                    if cur_gt_names[j] == 'Car' or cur_gt_names[j] == 'Pedestrian' or cur_gt_names[j] == 'Cyclist':
                                        wrong_class_detections[cur_gt_names[j]] += 1
    ...
    # Print the number of wrong class detections for each class
    for cls, count in wrong_class_detections.items():
        print(f"Class: {cls}")
        print(f"  Wrong class detections: {count}")
        
    # Print the overall results
    # total num of detections
    print (f'\n==========all boxes==========')
    print(len(eval_aos_results))
...
def main(args):
    ...
    # pcd_limit_range = np.array([0, -15, -3, 70.4, 15, 0.0], dtype=np.float32)
