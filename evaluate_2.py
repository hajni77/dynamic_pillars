...
def get_score_thresholds(tp_scores, total_num_valid_gt, num_sample_pts=41):
    score_thresholds = []
    tp_scores = sorted(tp_scores)[::-1] # descending order
            ...
            for difficulty in [0, 1, 2]:
                ...
                for id in ids:
                    ...
                    for j, cur_gt_name in enumerate(cur_gt_names):
                        ...
                        # Treat Cyclist and Pedestrian as Human
                        if cur_gt_name in ['Cyclist', 'Pedestrian']:
                            valid_class = 1 if cls == 'Human' else -1
                        elif cur_gt_name == cls:
                            valid_class = 1
                    ...
                    for j, cur_det_name in enumerate(cur_det_names):
                        ...
                        # Treat Cyclist and Pedestrian as Human
                        if cur_det_name in ['Cyclist', 'Pedestrian']:
                            det_ignores.append(0 if cls == 'Human' else -1)
...
def main(args):
    ...
    # Add this mapping for merging Cyclist and Pedestrian into Human
    LABEL2CLASSES = {v: ('Human' if k in ['Cyclist', 'Pedestrian'] else k) for k, v in CLASSES.items()}
    if not args.no_cuda:
    ...
    pcd_limit_range = np.array([0, -40, -2, 73, 40, 0.0], dtype=np.float32)
    model.eval()
    with torch.no_grad():
        format_results = {}
