...
class DynamicConv(nn.Module):
    ...
    def forward(self,x):
        ...
        output = output.view(bs, self.out_planes, output.size(2), output.size(3))
        
        return output

class Backbone(nn.Module):
    def __init__(self, in_channel, out_channels, layer_nums, layer_strides=[2, 2, 2, 2]):
        super().__init__()
        ...
        for i in range(len(layer_strides)):
            blocks = []
            blocks.append(DynamicConv(in_channel, out_channels[i], 3, stride=layer_strides[i], padding=1, bias=False))
            blocks.append(nn.BatchNorm2d(out_channels[i], eps=1e-3, momentum=0.01))
            blocks.append(nn.ReLU(inplace=True))

            for _ in range(layer_nums[i]):
                blocks.append(nn.Conv2d(out_channels[i], out_channels[i], 3, padding=1,bias=False))
                blocks.append(nn.BatchNorm2d(out_channels[i], eps=1e-3, momentum=0.01))
                blocks.append(nn.ReLU(inplace=True))
...
class PointPillars(nn.Module):
    def __init__(self,
                 ...
                 max_num_points=70,  # increase the number of points per voxel-increase memory and comp. cost.
        ...
        self.backbone = Backbone(in_channel=64, 
                                 ...
                                 layer_nums=[3, 5, 5, 5])
        self.neck = Neck(in_channels=[64, 128, 256,512], 
                         upsample_strides=[1, 2, 4,8], 
                         out_channels=[128, 128, 128,128])
        self.head = Head(in_channel=512, n_anchors=2*nclasses, n_classes=nclasses)
        ...
        # anchors
        # ranges = [[5.33, -13.77, -0.6, 72.4943, 18.3772, -0.6],
        #             [5.33, -13.77, -0.6, 72.4943, 18.3772, -0.6],
        #             [5.33, -13.77, -1.6, 72.4943, 18.3772, -1.6]]
