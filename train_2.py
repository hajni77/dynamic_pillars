...
def main(args):
    ...
    for epoch in range(args.max_epoch):
        ...
        # Safely convert to a tensor if needed
        if isinstance(batched_gt_bboxes, list):
            if all(torch.is_tensor(bbox) for bbox in batched_gt_bboxes):
                # Convert list of tensors to a single tensor
                batched_gt_bboxes = torch.stack(batched_gt_bboxes)
            else:
                raise ValueError("batched_gt_bboxes is not a list of tensors.")

        if isinstance(batched_labels, list):
            batched_labels = torch.tensor(batched_labels).cuda()

            # Define the valid range for your pseudo-LiDAR point cloud
            x_min, y_min, z_min, x_max, y_max, z_max = [0, -14, -2, 73, 19, 0.0]

            # Create a mask to filter out bounding boxes outside the range
            mask = torch.logical_and(
                torch.logical_and(batched_gt_bboxes[:, 0] >= x_min, batched_gt_bboxes[:, 0] <= x_max),
                torch.logical_and(
                    torch.logical_and(batched_gt_bboxes[:, 1] >= y_min, batched_gt_bboxes[:, 1] <= y_max),
                    torch.logical_and(batched_gt_bboxes[:, 2] >= z_min, batched_gt_bboxes[:, 2] <= z_max)
                )
            )
            # Apply the filtering to ground truth bounding boxes and labels
            batched_gt_bboxes = batched_gt_bboxes[mask]
            batched_labels = batched_labels[mask]
            ...
            loss.backward()
            # torch.nn.utils.clip_grad_norm_(pointpillars.parameters(), max_norm=35) //before backward
            optimizer.step()
            scheduler.step()
